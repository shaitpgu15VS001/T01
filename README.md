...
- **צריבת כתוביות קיימות**: מצב burn_only — מעלים סרטון + קובץ כתוביות מוכן ומקבלים סרטון עם כתוביות צרובות
...
|--------|-------|------|------------|
| `mode` | מצב: `full` (תמלול+תרגום+צריבה) / `burn_only` (צריבת כתוביות קיימות) | ❌ | `full` |
| `video_url` | קישור ישיר לסרטון *או* קישור שיתוף Dropbox | ✅ | — |
| `subtitle_url` | קישור לקובץ כתוביות (SRT/VTT/ASS) — למצב burn_only | ❌ | — |
...
| `font_size` | גודל גופן | ❌ | `20` |
| `font_color` | צבע טקסט בהקס | ❌ | `&H00FFFFFF` |
| `position` | מיקום: `bottom` / `top` | ❌ | `bottom` |
| `translation_engine` | מנוע תרגום: `argos` / `google` | ❌ | `argos` |
| `whisper_model` | דגם תמלול: `tiny` / `base` / `small` / `medium` | ❌ | `tiny` |
...
### מצב `burn_only`
- `סרטון-מסומן.mp4` — הסרטון עם הכתוביות שהעליתם
